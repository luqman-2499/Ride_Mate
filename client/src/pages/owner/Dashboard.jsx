...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (resData.success) {
        ...
        toast.error(resData.message || 'Failed to load dashboard')
      }
    } catch (error) {
  ...
  return (
    <div className='px-4 pt-10 md:px-10 flex-1'>
      <Title title='Admin Dashboard' subTitle="Monitor overall platform performances including total cars, bookings and other activities" />
            ...
            return (
              <div key={booking?._id || index} className='mt-4 flex items-center justify-between'>
                <div className='flex items-center gap-2'>
                  ...
                  <div>
                    <p>{car.brand ?? 'Deleted'} {car.model ?? ''}</p>
                    <p className='text-sm text-gray-500'>{booking?.createdAt ? String(booking.createdAt).split('T')[0] : '-'}</p>
                  </div>
                </div>

                <div className='flex items-center gap-2 font-medium'>
                  <p className='text-sm text-gray-500'>{currency}{booking?.price ?? 0}</p>
                  <p className='px-3 py-0.5 border border-borderColor rounded-full text-sm'>{booking?.status ?? 'unknown'}</p>
                </div>
              </div>
            )
          })}
