...
const ManageBookings = () => {
  ...
  const fetchOwnerBookings = async ()=>{
    try {
      ...
      data.success ? setBookings(data.bookings) : toast.error(data.mesage)
    } catch (error) {
              ...
              <tr key={index} className='border-t border-borderColor text-gray-500'>
                ...
                <td className='p-3'>
                  {booking.status === 'pending' ? (
                    ...
                    <span className={`px-3 py-1 rounded-all text-xs font-semibold ${booking.status === 'confirmed' ? 'bg-green-100 text-green-500' : 'bg-red-100 text-red-500'}`}>{booking.status}</span>
                  ) }
                </td>
              </tr>
